...
public record CreateEmployeeCommand(Guid OrganizationId, string FullName, string Email, string Password, int Role, List<Guid>? WorkspaceIds, Guid CallerId) : IRequest<Result<OrgMemberDto>>;
public record UpdateEmployeeCommand(Guid OrganizationId, Guid UserId, string? FullName, string? Email, int? Role, List<Guid>? WorkspaceIds, Guid CallerId) : IRequest<Result<OrgMemberDto>>;
public record CreateEmployeeWithRolesCommand(Guid OrganizationId, string FullName, string Email, string Password, List<Identity.Service.Application.Interfaces.WorkspaceRoleAssignment> WorkspaceRoles, Guid CallerId, int OrgRole) : IRequest<Result<OrgMemberDto>>;
public record UpdateEmployeeWithRolesCommand(Guid OrganizationId, Guid UserId, string? FullName, string? Email, List<Identity.Service.Application.Interfaces.WorkspaceRoleAssignment>? WorkspaceRoles, Guid CallerId, int? OrgRole = null) : IRequest<Result<OrgMemberDto>>;
...
public class CreateEmployeeWithRolesHandler : IRequestHandler<CreateEmployeeWithRolesCommand, Result<OrgMemberDto>>
{
    ...
    {
        try { var dto = await _service.CreateEmployeeWithRolesAsync(req.OrganizationId, req.FullName, req.Email, req.Password, req.WorkspaceRoles, req.CallerId, req.OrgRole, ct); return Result<OrgMemberDto>.Success(dto); }
        catch (Exception ex) { return Result<OrgMemberDto>.Failure(ex.Message); }
    }
}
...
public class UpdateEmployeeWithRolesHandler : IRequestHandler<UpdateEmployeeWithRolesCommand, Result<OrgMemberDto>>
{
    ...
    {
        try { var dto = await _service.UpdateEmployeeWithRolesAsync(req.OrganizationId, req.UserId, req.FullName, req.Email, req.WorkspaceRoles, req.CallerId, req.OrgRole, ct); return Result<OrgMemberDto>.Success(dto); }
        catch (Exception ex) { return Result<OrgMemberDto>.Failure(ex.Message); }
    }
}
