...
public class SubTaskService : ISubTaskService
{
    ...
    {
        if (callerRoles.Contains("Viewer") || callerRoles.Contains("Client"))
        ...
        _db.ActivityLogs.Add(new Domain.Entities.ActivityLog(task.ProjectId, taskId, callerId, "SubTaskCreated", $"{{\"title\":\"{title}\"}}"));
    ...
    public async Task<Result<bool>> DeleteSubTaskAsync(Guid subTaskId, Guid callerId, CancellationToken ct = default)
    {
        ...
        if (task != null) _db.ActivityLogs.Add(new Domain.Entities.ActivityLog(task.ProjectId, sub.TaskId, callerId, "SubTaskDeleted", $"{{\"title\":\"{sub.Title}\"}}"));
        await _db.SaveChangesAsync(ct);
        if (task != null) await _cache.RemoveAsync(CacheKeys.Board(task.ProjectId));
