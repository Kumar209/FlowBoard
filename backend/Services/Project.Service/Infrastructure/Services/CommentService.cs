...
public class CommentService : ICommentService
{
    ...
    public async Task<Result<CommentDto>> AddCommentAsync(Guid taskId, string content, Guid callerId, List<string> callerRoles, CancellationToken ct = default)
    {
        ...
        var evt = new { TaskId = taskId, ProjectId = task.ProjectId, CommentId = comment.Id, ActorId = callerId, OccurredOnUtc = DateTime.UtcNow, EventId = Guid.NewGuid(), CorrelationId = Guid.NewGuid().ToString() };
