...
/// </summary>
public record GetTasksQuery(Guid ProjectId, string? Search, Guid? AssigneeId, string? Priority, string? Label, DateTime? DueFrom, DateTime? DueTo, string? SortBy, bool SortDesc, int Page, int PageSize, string? SprintId = null) : ICacheableRequest<PaginatedResult<TaskDto>>
{
    public string CacheKey
    {
        get
        {
            var raw = $"{ProjectId}:{Search}:{AssigneeId}:{Priority}:{Label}:{DueFrom}:{DueTo}:{SortBy}:{SortDesc}:{Page}:{PageSize}:{SprintId}";
...
public class GetTasksHandler : IRequestHandler<GetTasksQuery, PaginatedResult<TaskDto>>
{
    ...
    public Task<PaginatedResult<TaskDto>> Handle(GetTasksQuery req, CancellationToken ct)
        => _service.GetTasksAsync(req.ProjectId, req.Search, req.AssigneeId, req.Priority, req.Label, req.DueFrom, req.DueTo, req.SortBy, req.SortDesc, req.Page, req.PageSize, req.SprintId, ct);
}
