...
## Pre-Deploy Bug Fixes (10) — Approved Approaches 2026-09-15 (One at a Time, No Commit Without Approval)

> Logged before deployment `49/50`. Each bug will be fixed one at a time, tested in UI by you, then committed with `fix: bug #N ...`. This section is the single source so future sessions remember chosen options. No extra API calls where noted.

| # | Bug | Approved Option | Extra API? | UI Change? | Summary |
|---|-----|-----------------|------------|------------|---------|
| 1 | Org System static | **B** | No new table — reuse `GET /api/superadmin/system` via new `GET /api/organizations/{id}/system` proxy (check `IsOrgAdmin`) | Yes — replace 5 static cards with `@for services`, `healthy/total`, `checkedAt`, per-service `status/latency` + error modal (same as `features/superadmin/system`) | Org `SystemComponent` will `injectQuery ['org-system',orgId] => orgService.getSystem(orgId)` → backend proxy calls `ISuperAdminService.GetSystemStatusAsync` after `IsOrgAdmin` check. No duplication of `SuperAdminService` health list (10 services). |
| 2 | Support paginator | **B** | Yes — paginated `GET /api/organizations/{id}/complaints?page&pageSize` + `GET /api/superadmin/complaints?orgId&page&pageSize` returns `{items,total,page,pageSize}` | Yes — `page/pageSize signals`, `Prev/Next`, `Showing x-y of total`, `pageSize selector 10/20` like `organizations` | Backend `Skip((page-1)*pageSize).Take(pageSize)` + `CountAsync`, service `getComplaints(orgId,page,pageSize)`, component `queryKey ['complaints',orgId,page]`. |
| 3 | Backlog paginator | **B** | Yes — server filter `GET /api/tasks?projectId&sprintId=null&page&pageSize&search` (backlog = `SprintId IS NULL`) | Yes — `page/pageSize` + `Showing filtered/total` + `Prev/Next` | Backend add `sprintId` nullable string param (`null` → `WHERE SprintId IS NULL`, `guid` → `WHERE SprintId=guid`), `GetTasksQuery SprintId`. Frontend `backlogQuery = injectQuery(['backlog',pid,page,search] => ps.getTasks(pid,{sprintId:'null',page,pageSize,search}))`. No longer `boardQuery.data().tasks.filter`. |
| 4 | Org member delete cascade | **A** | No new API — harden `OrganizationService.DeleteEmployeeAsync` | No UI | Full cascade in `IdentityDbContext` + raw `[project]` SQL: `DELETE OrganizationMembers`, `DELETE WorkspaceMembers where OrganizationId`, `DELETE ProjectMembers WHERE ProjectId IN (SELECT WorkspaceId...)`, `UPDATE TaskItems SET AssigneeId=NULL WHERE AssigneeId=uid`, `DELETE TeamMembers`, `Remove pending`. Single transaction (like `SuperAdminService.DeleteUser`). Optional MassTransit event later. |
| 5 | WorkStatus KPI dynamic | **A with constraint — fit in overview API, no extra call** | **No extra `GET /statuses`** — enrich `GET /api/projects/{id}/overview` or `GET /api/projects/{id}/stats` to include `workStatus: [{statusId,name,count,color}]` | Yes — replace 3 static `To Do/In Review/Done` `@for s of workStatus` | Backend `ProjectStatsService.GetOverviewAsync` add `workStatus = _db.Statuses Where ProjectId GroupJoin Tasks GroupBy StatusId Count` (`Statuses` explicit per project). Frontend `overview.component html:95` `@for s of overview.workStatus track s.statusId` dynamic cards. If no statuses, show `No statuses — create in Project Settings → Statuses`. |
| 6 | Sprint burndown wrong boards API | **B** | No new API — reuse `GET /api/projects/{id}/burndown?sprintId` | Yes — sprint `<select>` + header `SprintName • start→end • Total N` | Frontend `sprintsQuery` + `selectedSprintId = Active || last` `burndownQuery key ['burndown',pid,sid] => stats.getBurndown(pid,sid)` + `sprints.component` style selector. Backend already returns `BurndownDto(sprintId,sprintName,start,end,points)`. Overview will show chosen sprint context. |
| 7 | Boards tasks `0` static | **B with constraint — don't create another API call** | **No new endpoint** — enrich existing `GET /api/projects/{id}/boards` `BoardInfoDto` with `TaskCount` | No new UI shape — `{{b.taskCount}} tasks` | Backend `BoardService.GetBoardsAsync` add `TaskCount = Tasks.Count(t=>t.ProjectId==pid && (board filter matches))` per board: if `filterJson teamIds` empty → all project tasks filtered by `BoardColumnStatus` mapping (`StatusIds` per board), else respect `teamIds`. Frontend `displayBoards` remove `tasks:0`, use `b.taskCount`. One API, one DTO field. |
| 8 | OrgAdmin promotion retains CustomRoleId 403 | **A + C + B backfill** | No new API | No UI | `A`: Harden `UpdateEmployeeWithRolesAsync` always `CustomRoleId=null` for all `WorkspaceMembers` in org when `orgRole==OrgAdmin` before handling `workspaceRoles`. `C`: Caller `members.component` pass `workspaceRoles:null` when Role=OrgAdmin. `B`: Backfill migration `UPDATE [identity].WorkspaceMembers SET CustomRoleId=NULL WHERE UserId IN (SELECT UserId FROM OrganizationMembers WHERE Role=2)` + startup `IdentitySeeder` fix for existing promoted users. |
| 9 | Maintenance loader every 60s | **A — silent** | No API change | No UI | Verify `layout.component.ts:59` `headers {'X-Silent':'true'}` `setInterval 300000 (5m)` + `document.visibilityState==='visible'` + `loading.interceptor.ts:9` `silentUrls=['/api/platform/maintenance','/api/platform/general','/api/feature-flags']` + `req.headers.has('X-Silent')` skips `loading.show()`. Fix: ensure `60000` → `300000`, ensure `X-Silent` present, ensure `silentUrls` check before `show`. Must be silent (no spinner). |
