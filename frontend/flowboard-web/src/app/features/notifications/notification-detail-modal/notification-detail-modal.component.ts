...
export class NotificationDetailModalComponent {
  ...
  payload = computed(() => {
    try {
      return JSON.parse(this.notification()?.payloadJson || '{}');
    } catch {
      return {};
    }
  });
  ...
  prettyPayload = computed(() => {
    ...
    Object.keys(obj).forEach(k => {
      if (!obj[k] || obj[k].toString().trim() === '') delete obj[k];
    });
  ...
  humanMessage = computed(() => {
    ...
    return title
      ? `${actor}${role} ${displayAction} "${title}"${boardPart}${sprintPart}`
      : `${actor || 'Someone'}${role} ${displayAction}${boardPart}${sprintPart}`;
  });
