...
export class BoardsComponent {
    ...
    if (api && api.length > 0) {
      return api.map((b: any) => {
        ...
        try {
          ...
          if (f?.teamIds?.length) {
            filterInfo = ` • Team filter ${f.teamIds.length}`;
          }
        } catch {}
  ...
  createMutation = injectMutation(() => ({
    mutationFn: () => {
      const filter = this.selectedTeam()
        ? JSON.stringify({ teamIds: [this.selectedTeam()] })
        : null;
    ...
    onSuccess: () => {
      this.qc.invalidateQueries({ queryKey: ['boards', this.projectId()] });
  ...
  deleteMutation = injectMutation(() => ({
    ...
    onError: (e: any) => {
      this.toast.error(e.error?.error || e.message || 'Delete failed');
      this.deleteTarget.set(null);
    }
  }));
  ...
  openEdit(b: any) {
    this.editingBoard.set(b);
    this.newName.set(b.name);
    this.newType.set(b.type);
    ...
    try {
      ...
      const matched = teams.find(
        (t: any) => t.id.toLowerCase() === rawId.toLowerCase()
      );
