...
export class TeamComponent {
  ...
  filteredTeams = computed(() => {
    ...
    return s
      ? list.filter((t: any) => t.name.toLowerCase().includes(s))
      : list;
  });
    ...
    mutationFn: () =>
      firstValueFrom(
        this.ps.createTeam(
          ...
          this.newTeamDesc().trim() || undefined
        )
      ),
    onSuccess: () => {
  ...
  addMemberMutation = injectMutation(() => ({
    ...
    onSuccess: () => {
      this.qc.invalidateQueries({
        queryKey: ['team-members', this.selectedTeamId()]
      });
      this.qc.invalidateQueries({
  ...
  openEdit(team: any) {
    ...
    this.newTeamName.set(team.name);
    this.newTeamDesc.set(team.description || '');
    this.showCreate.set(true);
  }
  ...
  filteredWorkspaceMembers = computed(() => {
    ...
    const available = list.filter(
      (m: any) => !existingIds.has(m.userId)
    );
