      ...
      if (auth.accessToken()) {
        if ((!auth.currentUser() || auth.memberships().length === 0) && res?.accessToken) {
          try {
            ...
            const name = payload['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'] || payload.name || payload.unique_name || (email ? email.split('@')[0] : 'User');
            if (sub && !auth.currentUser()) auth.currentUser.set({ id: sub, email, fullName: name });
            ...
            if (auth.memberships().length === 0) {
              ...
              const roles: any = payload.role || payload['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'] || payload[Object.keys(payload).find(k => k.toLowerCase().includes('role')) as string] || [];
              ...
              if (widArr.length || roleArr.length || isSuper) {
                ...
                for (let i = 0; i < max; i++) {
                  ...
                  const r = roleArr[i] || roleArr[0] || (isSuper ? 'SuperAdmin' : 'Member');
                  memberships.push({ workspaceId: wid, role: r, roleName: typeof r === 'string' ? r : undefined });
                }
                if (isSuper && !memberships.some(m => m.role === 'SuperAdmin' || m.roleName === 'SuperAdmin')) {
                  memberships.push({ workspaceId: '00000000-0000-0000-0000-000000000000', role: 0, roleName: 'SuperAdmin' });
                }
