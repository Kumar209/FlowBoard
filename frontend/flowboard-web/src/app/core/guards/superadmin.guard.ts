  ...
  if (!auth.isAuthenticated()) {
    ...
    try {
      ...
      if (res?.accessToken) {
        ...
        try { const me: any = await firstValueFrom(auth.me()); auth.hydrateFromMe(me); } catch {}
        ...
        if (auth.memberships().length === 0 && res.accessToken) {
          try {
            ...
            const roles: any = payload.role || payload['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'] || [];
            const roleArr = Array.isArray(roles) ? roles : (roles ? [roles] : []);
            if (isSuper || roleArr.includes('SuperAdmin')) {
              auth.memberships.set([{ workspaceId: '00000000-0000-0000-0000-000000000000', role: 0, roleName: 'SuperAdmin' } as any]);
            }
          } catch {}
        }
      }
